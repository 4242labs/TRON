...
# ADR-0003 D-G (block 02-12 T1): `--show-toplevel` resolves the CURRENT working
# tree's own root — from inside a linked worktree (exactly where a worker runs this
# script, per the carve-your-own-worktree ritual) that is the WORKTREE's root, not
# the shared project root the grants dir lives under. `--git-common-dir` instead
# resolves to the ONE .git directory every worktree of this repo shares (main
# checkout or any linked worktree alike), so its parent is the true, worktree-safe
# project root regardless of which worktree land.sh is invoked from.

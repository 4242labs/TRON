...
Standard format for code review reports written to `logs/review-code/YYMMDD-HHMM-{scope}.md`.
Fillable template — not a procedure.
...
|:---|:---------|:-----|:--------|:-------|
| CR-H1 | BLOCKER / HIGH / MEDIUM / LOW / INFO | `path/to/file.ext:line` | {one-line description} | open / fixed-in-session / deferred-with-approval |
...
```
**Why it matters:** {concrete impact — correctness bug, performance, maintainability}
**Recommendation:** {specific fix — code-level when possible}
**References:** {prior CR IDs, related findings}
